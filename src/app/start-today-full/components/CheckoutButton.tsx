...
export function CheckoutButton({
  ...
  const handleClick = async () => {
    setStatus("loading");
    track("Start Today Full Checkout Started", {
      Placement: placement,
    ...
    try {
      const res = await fetch("/api/start-today-full-checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          source: "start-today-full",
          promoCode,
      ...
      if (!res.ok || !data.url) {
        setStatus("error");
        track("Start Today Full Checkout Error", {
          Placement: placement,
          "Error Message": data.error,
        });
        return;
      }
      track("Start Today Full Checkout Redirected", { Placement: placement });
