...
// Its own route (same pattern as /api/start-today-checkout and
...
const START_TODAY_FULL_CHANNEL = "start_today_full_web_payment";
...
export async function POST(request: NextRequest) {
  ...
  attributionMetadata.channel = START_TODAY_FULL_CHANNEL;
  ...
  const metadata: Record<string, string> = { ...attributionMetadata, source: "start-today-full" };
  const promoCode =
  ...
  const cancelUrl = new URL("/start-today-full", origin);
  ...
  try {
    ...
    console.log(
      "Start Today Full checkout session params:",
